""" NANODEZ Telegram buyurtma-boti — to'liq versiya -------------------------------------------------- Funksiyalar: - Buyurtma qabul qilish (ism, zararkunanda turi, manzil, telefon) - "Biz haqimizda" va "Kafolat" bo'limlari - Buyurtma holatini kuzatish (Yangi -> Jarayonda -> Bajarildi) - Operatorlar guruhida /holat buyrug'i orqali holatni yangilash - Ish "Bajarildi" bo'lganda mijozga QR-kodli shartnoma (PDF) avtomatik yuboriladi Ishga tushirish: 1. pip install -r requirements.txt 2. python bot.py """
...
def init_db():
    ...
    conn.execute(
        """ CREATE TABLE IF NOT EXISTS orders ( order_id TEXT PRIMARY KEY, user_id INTEGER, username TEXT, name TEXT, pest_type TEXT, address TEXT, phone TEXT, status TEXT DEFAULT 'yangi', created_at TEXT, amount TEXT, warranty TEXT ) """
    )
    conn.commit()
    conn.close()
